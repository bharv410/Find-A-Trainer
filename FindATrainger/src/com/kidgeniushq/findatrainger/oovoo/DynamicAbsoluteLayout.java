...
public class DynamicAbsoluteLayout extends ViewGroup {
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				if (child.getVisibility() != GONE) {
					...
					DynamicAbsoluteLayout.LayoutParams lp = (DynamicAbsoluteLayout.LayoutParams) child
							.getLayoutParams();
	...
	public ViewGroup.LayoutParams generateLayoutParams(AttributeSet attrs) {
		return new DynamicAbsoluteLayout.LayoutParams(getContext(), attrs);
	}

	// Override to allow type-checking of LayoutParams.
	@Override
	protected boolean checkLayoutParams(ViewGroup.LayoutParams p) {
		return p instanceof DynamicAbsoluteLayout.LayoutParams;
	}
